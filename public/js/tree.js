...
(function (FT) {
  ...
  async function render(main, params) {
    ...
    const renderer = FT.TreeRenderer.create(stage, {
      ...
      // Parent Expansion Indicator: reveal a card's hidden parents in place.
      onExpandParents: (id) => {
        ui.expanded.add(id);
        if (ui.cfg.up < 8) { ui.cfg.up += 1; if (upCtl.set) upCtl.set(ui.cfg.up); }
    ...
    async function refresh(o) {
      o = o || {};
      const graph = await Tree.focusGraph(ui.focusId, ui.cfg.up, ui.cfg.down);
      if (!graph) return;
